  ...
  public function index(Request $request)
{
    ...
    // ==========================================
    // 🔥 DAYWISE COLLECTION DATA (Last 30 Days)
    // ==========================================
    $daywiseLabels = [];
    $daywiseData = [];
    ...
    for ($i = 29; $i >= 0; $i--) {
        ...
        $daywiseLabels[] = $date->format('d M');

        $dailyRevenue = PaymentHistory::whereDate('payment_date', $dateString)->sum('amount') ?? 0;
        $daywiseData[] = $dailyRevenue;
    }

    // ==========================================
    // 🔥 DAYWISE PAYMENT HISTORY (Last 30 Days)
    // ==========================================
  // ==========================================
// DAYWISE PAYMENT HISTORY (Today)
// ==========================================
$daywisePayments = PaymentHistory::with('member')
    ->whereDate('payment_date', today())
    ->orderBy('payment_date', 'desc')
    ->get();

$daywiseTotal = $daywisePayments->sum('amount');

    // ==========================================
    // 🔥 MONTHWISE PAYMENT HISTORY (Last 12 Months)
    // ==========================================
    $monthwisePayments = PaymentHistory::with('member')
        ->whereYear('payment_date', '>=', now()->subMonths(12)->year)
        ->whereMonth('payment_date', '>=', now()->subMonths(12)->month)
        ->orderBy('payment_date', 'desc')
        ->get();

    // ==========================================
    // 🔥 MONTHWISE TOTAL COLLECTION
    // ==========================================
    $monthwiseTotal = $monthwisePayments->sum('amount');
