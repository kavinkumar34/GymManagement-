...
<style>
    ...
    .table-brands thead th.text-center {
        text-align: center;
    }
    ...
    .table-brands tbody tr:last-child td {
        border-bottom: none;
    }
    ...
    .table-brands .brand-placeholder {
        width: 40px;
        height: 40px;
        border-radius: var(--radius);
        background: var(--light-gray);
    ...
    .table-brands .brand-name {
        font-weight: 600;
        color: var(--dark);
    }

    .table-brands .brand-desc {
        font-size: 12px;
        color: var(--gray);
        max-width: 200px;
    ...
    .table-brands .status-badge.active {
        background: #e8f5e9;
        color: #2e7d32;
    }

    .table-brands .status-badge.inactive {
        background: #fce4ec;
        color: #c62828;
    }

    .table-brands .status-badge .dot {
        width: 6px;
        height: 6px;
        border-radius: 50%;
        display: inline-block;
    ...
    .table-brands .status-badge.active .dot {
        background: #4caf50;
    }

    .table-brands .status-badge.inactive .dot {
        background: #ef5350;
    }
...
<div class="admin-main-content">
    <div class="list-card">
        <div class="card-header">
            <div>
                <h4><i class="fas fa-building"></i> Brands</h4>
                <small style="opacity:0.8;">Manage all brands</small>
            </div>
            <div style="display:flex; gap:8px; flex-wrap:wrap; align-items:center;">
                <a href="{{ route('admin.brands.create') }}" class="btn btn-primary" style="background:#4a9eff; color:#fff; border:none; padding:7px 16px; border-radius:10px; text-decoration:none; display:inline-flex; align-items:center; gap:6px; font-weight:500; font-size:12px; transition:all 0.3s;">
                    <i class="fas fa-plus"></i> Add Brand
                </a>
                <span style="background:rgba(74,158,255,0.2); color:#8ab4f8; padding:3px 12px; border-radius:50px; font-size:11px;">
                    <i class="fas fa-tag"></i> Total: {{ $brands->total() }}
                </span>
            </div>
        </div>
        ...
        <div class="card-body">
            ...
            <div class="table-responsive">
                <table class="table-brands" id="brandsTable">
                    <thead>
                        <tr>
                    ...
                    <tbody id="tableBody">
                        ...
                        <tr>
                            <td class="text-center sno">{{ $brands->firstItem() + $index }}</td>
                            <td>
                                @if($brand->logo)
                            ...
                            <td>
                                <span class="brand-desc" title="{{ $brand->description }}">{{ Str::limit($brand->description, 50) ?? '-' }}</span>
                            </td>
                            <td class="text-center">
                                <span class="status-badge {{ $brand->is_active ? 'active' : 'inactive' }}">
                                    <span class="dot"></span> {{ $brand->is_active ? 'Active' : 'Inactive' }}
                                </span>
                            </td>
                            <td>
                                <div class="action-btns">
                                    <a href="{{ route('admin.brands.edit', $brand->id) }}" class="btn-action edit" title="Edit">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <button type="button" class="btn-action delete" onclick="openDeleteModal({{ $brand->id }})" title="Delete">
                        ...
                        <tr>
                            <td colspan="6">
                                <div class="empty-state">
                                    ...
                                    <p><a href="{{ route('admin.brands.create') }}">Create your first brand!</a></p>
                                </div>
                            </td>
                        </tr>
                        @endforelse
            ...
            <div class="pagination-wrapper">
                <div class="pagination-info">
                    Showing <strong>{{ $brands->firstItem() ?? 0 }}</strong> to <strong>{{ $brands->lastItem() ?? 0 }}</strong> of <strong>{{ $brands->total() ?? 0 }}</strong> entries
                </div>
...
document.getElementById('confirmDeleteBtn').addEventListener('click', function() {
    if (deleteId) {
        ...
        form.action = '/admin/brands/' + deleteId;
        form.submit();
    }
});
...
function filterTable() {
    ...
    if (allRows.length > 0 && allRows.length === visibleRows.length) {
        ...
        td.innerHTML = '<i class="fas fa-search" style="font-size:24px; display:block; margin-bottom:8px; color:#dee2e6;"></i> No brands found matching your filters.';
        tr.appendChild(td);
