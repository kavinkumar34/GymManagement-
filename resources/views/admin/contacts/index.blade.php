...
<style>
    ...
    .table-contacts thead th.text-center {
        text-align: center;
    }
    ...
    .table-contacts .contact-name {
        font-weight: 600;
        color: var(--dark);
    }

    .table-contacts .contact-email {
        font-size: 12px;
        color: var(--gray);
    }

    .table-contacts .subject-text {
        font-weight: 500;
        color: var(--dark);
    }

    .table-contacts .status-badge {
        padding: 4px 14px;
        border-radius: 50px;
        font-size: 11px;
        font-weight: 500;
    ...
    .table-contacts .status-badge.pending {
        background: #fef3c7;
        color: #92400e;
    }

    .table-contacts .status-badge.read {
        background: #e3f2fd;
        color: #1565c0;
    }

    .table-contacts .status-badge .dot {
        width: 6px;
        height: 6px;
        border-radius: 50%;
    ...
    .table-contacts .status-badge.pending .dot {
        background: #ffa726;
    }

    .table-contacts .status-badge.read .dot {
        background: #4a9eff;
    }

    .table-contacts .received-date {
        font-size: 12px;
        color: var(--gray);
        white-space: nowrap;
    }
    ...
    @media (max-width: 768px) {
        ...
        .action-btns { gap: 3px; }
        .action-btns .btn-action { width: 28px; height: 28px; font-size: 10px; }
        .table-contacts thead th { font-size: 10px; padding: 6px 8px; }
        .table-contacts tbody td { padding: 6px 8px; font-size: 11px; }
    ...
    @media (max-width: 576px) {
        ...
        .table-contacts tbody td { padding: 4px 6px; font-size: 10px; }
        .table-contacts thead th { padding: 4px 6px; font-size: 9px; }
        .action-btns .btn-action { width: 24px; height: 24px; font-size: 9px; }
        .table-contacts .status-badge { font-size: 9px; padding: 2px 8px; }
    ...
    <div class="list-card">
        <div class="card-header">
            <div>
                ...
                <small style="opacity:0.8;">Manage all contact form messages</small>
            </div>
            <span style="background:rgba(74,158,255,0.2); color:#8ab4f8; padding:3px 12px; border-radius:50px; font-size:11px;">
                <i class="fas fa-inbox"></i> Total: {{ $contacts->total() }}
            </span>
        </div>
        ...
        <div class="card-body">
            ...
            <div class="search-filter-section">
                <div class="search-box">
                    ...
                    <input type="text" id="searchInput" placeholder="Search by name, email, subject..." onkeyup="filterTable()">
                </div>
            ...
            <div class="table-responsive">
                <table class="table-contacts" id="contactsTable">
                    <thead>
                        <tr>
                    ...
                    <tbody id="tableBody">
                        ...
                        <tr>
                            <td class="text-center sno">{{ $contacts->firstItem() + $index }}</td>
                            <td>
                            ...
                            <td><span class="subject-text">{{ Str::limit($contact->subject, 50) }}</span></td>
                            <td class="text-center">
                                <span class="status-badge {{ strtolower($contact->status) }}">
                                    <span class="dot"></span> {{ $contact->status }}
                                </span>
                            </td>
                            <td class="text-center"><span class="received-date">{{ $contact->created_at->format('d M Y, h:i A') }}</span></td>
                            <td>
                                <div class="action-btns">
                                    <a href="{{ route('admin.contacts.show', $contact->id) }}" class="btn-action view" title="View">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <button type="button" class="btn-action delete" onclick="openDeleteModal({{ $contact->id }})" title="Delete">
            ...
            <div class="pagination-wrapper">
                <div class="pagination-info">
                    Showing <strong>{{ $contacts->firstItem() ?? 0 }}</strong> to <strong>{{ $contacts->lastItem() ?? 0 }}</strong> of <strong>{{ $contacts->total() ?? 0 }}</strong> entries
                </div>
...
function filterTable() {
    ...
    if (allRows.length > 0 && allRows.length === visibleRows.length) {
        ...
        td.innerHTML = '<i class="fas fa-search" style="font-size:24px; display:block; margin-bottom:8px; color:#dee2e6;"></i> No messages found matching your filters.';
        tr.appendChild(td);
