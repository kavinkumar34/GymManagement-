...
<style>
    ...
    .table-producttypes thead {
        background: var(--light-gray);
    }

    .table-producttypes thead th {
        padding: 12px 14px;
        font-weight: 600;
        font-size: 11px;
    ...
    .table-producttypes thead th.text-center {
        text-align: center;
    }

    .table-producttypes tbody td {
        padding: 10px 14px;
        vertical-align: middle;
        border-bottom: 1px solid var(--border-color);
    }

    .table-producttypes tbody tr:hover {
        background: #f8f9fa;
    }

    .table-producttypes tbody tr:last-child td {
        border-bottom: none;
    }

    .table-producttypes .sno {
        font-weight: 600;
        color: var(--gray);
        font-size: 12px;
        text-align: center;
    ...
    .table-producttypes .pt-placeholder {
        width: 40px;
        height: 40px;
        border-radius: var(--radius);
        background: var(--light-gray);
    ...
    .table-producttypes .pt-name {
        font-weight: 600;
        color: var(--dark);
    }

    .table-producttypes .category-badge {
        padding: 3px 12px;
        border-radius: 50px;
        font-size: 11px;
        font-weight: 500;
    ...
    .table-producttypes .type-badge.topwear {
        background: #fff3cd;
        color: #856404;
    }

    .table-producttypes .type-badge.bottomwear {
        background: #d6d8db;
        color: #1e2124;
    }

    .table-producttypes .type-badge.footwear {
        background: #fce4ec;
        color: #c62828;
    }

    .table-producttypes .status-badge {
        padding: 4px 14px;
        border-radius: 50px;
        font-size: 11px;
        font-weight: 500;
    ...
    .table-producttypes .status-badge.active {
        background: #e8f5e9;
        color: #2e7d32;
    }

    .table-producttypes .status-badge.inactive {
        background: #fce4ec;
        color: #c62828;
    }

    .table-producttypes .status-badge .dot {
        width: 6px;
        height: 6px;
        border-radius: 50%;
    ...
    .table-producttypes .status-badge.active .dot {
        background: #4caf50;
    }

    .table-producttypes .status-badge.inactive .dot {
        background: #ef5350;
    }
...
<div class="admin-main-content">
    <div class="list-card">
        <div class="card-header">
            <div>
                <h4><i class="fas fa-tag"></i> Product Types</h4>
                <small style="opacity:0.8;">Manage all product types</small>
            </div>
            <div style="display:flex; gap:8px; flex-wrap:wrap; align-items:center;">
                <a href="{{ route('admin.producttypes.create') }}" class="btn btn-primary" style="background:#4a9eff; color:#fff; border:none; padding:7px 16px; border-radius:10px; text-decoration:none; display:inline-flex; align-items:center; gap:6px; font-weight:500; font-size:12px; transition:all 0.3s;">
                ...
                <span style="background:rgba(74,158,255,0.2); color:#8ab4f8; padding:3px 12px; border-radius:50px; font-size:11px;">
                    <i class="fas fa-tags"></i> Total: {{ $productTypes->total() }}
                </span>
            </div>
        </div>
        ...
        <div class="card-body">
            ...
            <div class="table-responsive">
                <table class="table-producttypes" id="productTypesTable">
                    <thead>
                        <tr>
                    ...
                    <tbody id="tableBody">
                        @forelse($productTypes as $index => $pt)
                        <tr>
                            <td class="text-center sno">{{ $productTypes->firstItem() + $index }}</td>
                            <td>
                                @if($pt->image)
                                    <img src="{{ asset('storage/'.$pt->image) }}" class="pt-image" alt="{{ $pt->name }}">
                                @else
                            ...
                            <td>
                                ...
                                @php
                                    $typeClass = '';
                                    $typeLabel = '';
                                    if(strpos(strtolower($pt->name), 'top') !== false) {
                                        $typeClass = 'topwear';
                                        $typeLabel = 'Top Wear';
                                    } elseif(strpos(strtolower($pt->name), 'bottom') !== false) {
                                        $typeClass = 'bottomwear';
                                        $typeLabel = 'Bottom Wear';
                                    } elseif(strpos(strtolower($pt->name), 'foot') !== false) {
                                        $typeClass = 'footwear';
                                        $typeLabel = 'Foot Wear';
                                    }
                                @endphp
                                @if($typeClass)
                                    ...
                                    <span class="type-badge {{ $typeClass }}">
                                        <i class="fas fa-{{ $typeClass == 'topwear' ? 'tshirt' : ($typeClass == 'bottomwear' ? 'fa-pants' : 'shoe-prints') }}"></i>
                                        {{ $typeLabel }}
                                    </span>
                                @endif
                            </td>
                            <td>
                                <span class="category-badge">
                                    <i class="fas fa-folder"></i> {{ $pt->category->name ?? 'N/A' }}
                                </span>
                            </td>
                            <td class="text-center">
                                <span class="status-badge {{ $pt->is_active ? 'active' : 'inactive' }}">
                                    <span class="dot"></span> {{ $pt->is_active ? 'Active' : 'Inactive' }}
                                </span>
                            </td>
                            <td>
                                <div class="action-btns">
                                    <a href="{{ route('admin.producttypes.edit', $pt->id) }}" class="btn-action edit" title="Edit">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <button type="button" class="btn-action delete" onclick="openDeleteModal({{ $pt->id }})" title="Delete">
                        ...
                        <tr>
                            <td colspan="6">
                                <div class="empty-state">
                                    ...
                                    <p><a href="{{ route('admin.producttypes.create') }}">Create your first product type!</a></p>
                                </div>
                            </td>
                        </tr>
                        @endforelse
            ...
            <div class="pagination-wrapper">
                <div class="pagination-info">
                    Showing <strong>{{ $productTypes->firstItem() ?? 0 }}</strong> to <strong>{{ $productTypes->lastItem() ?? 0 }}</strong> of <strong>{{ $productTypes->total() ?? 0 }}</strong> entries
                </div>
...
document.getElementById('confirmDeleteBtn').addEventListener('click', function() {
    if (deleteId) {
        ...
        form.action = '/admin/producttypes/' + deleteId;
        form.submit();
    }
});
...
function filterTable() {
    ...
    if (allRows.length > 0 && allRows.length === visibleRows.length) {
        ...
        td.innerHTML = '<i class="fas fa-search" style="font-size:24px; display:block; margin-bottom:8px; color:#dee2e6;"></i> No product types found matching your filters.';
        tr.appendChild(td);
