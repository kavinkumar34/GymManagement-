                        ...
                        <tbody>
                            @forelse($productTypes as $pt)
                                <tr>
                                    ...
                                    <td>
                                        @if ($pt->image)
                                            <img src="{{ asset('storage/' . $pt->image) }}"
                                    ...
                                    <td>
                                        <a href="{{ route('admin.producttypes.edit', $pt->id) }}"
                                            class="btn btn-sm btn-outline-primary">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <button class="btn btn-sm btn-outline-danger ms-1"
                                            onclick="deleteItem({{ $pt->id }})">
                                            <i class="fas fa-trash"></i>
                                        </button>
                                        <form id="delete-form-{{ $pt->id }}"
                                            action="{{ route('admin.producttypes.destroy', $pt->id) }}" method="POST"
                                            style="display:none;">
                                            @csrf @method('DELETE')
                                        </form>
                                    </td>
