...
@section('content')
    <style>
        ...
        .trainers-card .card-header h4 i {
            color: #4a9eff;
        }
        ...
        .table-trainers thead {
            background: var(--light-gray);
        }

        .table-trainers thead th {
            padding: 12px 14px;
            font-weight: 600;
            font-size: 11px;
        ...
        .table-trainers thead th.text-center {
            text-align: center;
        }

        .table-trainers tbody td {
            padding: 10px 14px;
            vertical-align: middle;
            border-bottom: 1px solid var(--border-color);
        }

        .table-trainers tbody tr:hover {
            background: #f8f9fa;
        }

        .table-trainers tbody tr:last-child td {
            border-bottom: none;
        }
        ...
        .table-trainers .trainer-id-badge {
        ...
        .table-trainers .trainer-name {
            font-weight: 600;
            color: var(--dark);
        }

        .table-trainers .trainer-email {
            font-size: 12px;
            color: var(--gray);
        }

        .table-trainers .specialization-tag {
            padding: 3px 12px;
            border-radius: 50px;
            font-size: 11px;
            font-weight: 500;
        ...
        .table-trainers .specialization-tag i {
            font-size: 10px;
        }

        .table-trainers .experience-badge {
            font-weight: 500;
            color: var(--dark);
        }

        .table-trainers .assigned-badge {
            padding: 3px 12px;
            border-radius: 50px;
            font-size: 11px;
            font-weight: 500;
        ...
        .table-trainers .assigned-badge i {
            font-size: 10px;
        }

        .table-trainers .status-badge {
            padding: 4px 14px;
            border-radius: 50px;
            font-size: 11px;
            font-weight: 500;
        ...
        .table-trainers .status-badge.active {
            background: #e8f5e9;
            color: #2e7d32;
        }

        .table-trainers .status-badge.inactive {
            background: #fce4ec;
            color: #c62828;
        }

        .table-trainers .status-badge .dot {
            width: 6px;
            height: 6px;
            border-radius: 50%;
        ...
        .table-trainers .status-badge.active .dot {
            background: #4caf50;
        }

        .table-trainers .status-badge.inactive .dot {
            background: #ef5350;
        }
        ...
        <div class="trainers-card">
            ...
            <div class="card-header">
                <div>
                    ...
                    <small style="opacity:0.8;">Manage all fitness trainers</small>
                </div>
            ...
            <div class="card-body">
                ...
                <div class="search-filter-section">
                    <div class="search-box">
                        ...
                        <input type="text" id="searchInput" placeholder="Search by name, email, phone, trainer ID..."
                            onkeyup="filterTable()">
                    </div>
                    <div class="filter-group">
                        <select id="specializationFilter" onchange="filterTable()">
                ...
                <div class="table-responsive">
                    <table class="table-trainers" id="trainersTable">
                        <thead>
                            <tr>
                        ...
                        <tbody id="tableBody">
                            @forelse($trainers as $index => $trainer)
                                <tr>
                                    <td class="text-center sno">{{ $trainers->firstItem() + $index }}</td>
                                    <td><span class="trainer-id-badge">{{ $trainer->trainer_id }}</span></td>
                                    <td>
                                        <div>
                                            <span class="trainer-name">{{ $trainer->name }}</span>
                                            <small class="trainer-email d-block">{{ $trainer->email }}</small>
                                        </div>
                                    </td>
                                    <td>{{ $trainer->phone }}</td>
                                    <td><span class="specialization-tag"><i class="fas fa-tag"></i>
                                            {{ $trainer->specialization }}</span></td>
                                    <td><span class="experience-badge">{{ $trainer->experience }} years</span></td>
                                    <td><span class="assigned-badge"><i class="fas fa-users"></i>
                                            {{ $trainer->assigned_members ?? 0 }}</span></td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="action-btns">
                                            ...
                                            <button type="button" class="btn-action delete"
                                                onclick="openDeleteModal({{ $trainer->id }})" title="Delete">
                                ...
                                <tr>
                                    <td colspan="9">
                                        <div class="empty-state">
                                            ...
                                            <p>Click "Add Trainer" to register a new trainer.</p>
                                            <a href="{{ route('admin.trainer.create') }}" class="btn btn-primary"
                ...
                <div class="pagination-wrapper">
                    <div class="pagination-info">
                        Showing <strong>{{ $trainers->firstItem() ?? 0 }}</strong> to
                        <strong>{{ $trainers->lastItem() ?? 0 }}</strong> of
                        <strong>{{ $trainers->total() ?? 0 }}</strong> entries
                    </div>
    ...
    <script>
        ...
        document.getElementById('confirmDeleteBtn').addEventListener('click', function() {
            if (deleteId) {
                ...
                form.action = '/admin/trainers/' + deleteId;
                form.submit();
            }
        });
        ...
        function filterTable() {
            ...
            var specializationFilter = document.getElementById('specializationFilter').value.toLowerCase();
            ...
            rows.forEach(function(row) {
                ...
                var specialization = row.querySelector('td:nth-child(5)')?.textContent.toLowerCase() || '';
                var status = row.querySelector('td:nth-child(8)')?.textContent.toLowerCase() || '';

                var matchesSearch = text.includes(searchValue);
                var matchesSpecialization = specializationFilter === '' || specialization.includes(
            ...
            if (allRows.length > 0 && allRows.length === visibleRows.length) {
                ...
                td.innerHTML =
                    '<i class="fas fa-search" style="font-size:24px; display:block; margin-bottom:8px; color:#dee2e6;"></i> No trainers found matching your filters.';
                tr.appendChild(td);
