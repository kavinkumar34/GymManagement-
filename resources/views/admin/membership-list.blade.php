...
@section('content')
    <style>
        ...
        .table-membership thead th.text-center {
            text-align: center;
        }
        ...
        .table-membership tbody tr:hover {
            background: #f8f9fa;
        }

        .table-membership tbody tr:last-child td {
            border-bottom: none;
        }

        .table-membership .sno {
            font-weight: 600;
            color: var(--gray);
            font-size: 12px;
            text-align: center;
        ...
        .table-membership .plan-name {
            font-weight: 600;
            color: var(--dark);
        }

        .table-membership .duration-badge {
            padding: 3px 12px;
            border-radius: 50px;
            font-size: 11px;
            font-weight: 500;
        ...
        .table-membership .price-amount {
            font-weight: 500;
            color: var(--dark);
        }

        .table-membership .discount-tag {
            padding: 3px 10px;
            border-radius: 50px;
            font-size: 11px;
            font-weight: 500;
        ...
        .table-membership .discount-tag.flat {
            background: #fff3e0;
            color: #e65100;
        }

        .table-membership .discount-tag.percentage {
            background: #f3e5f5;
            color: #6a1b9a;
        }

        .table-membership .discount-tag.none {
            background: #f5f5f5;
            color: #9e9e9e;
        }

        .table-membership .final-price {
            font-weight: 700;
            color: #2e7d32;
            font-size: 14px;
        }

        .table-membership .status-badge {
            padding: 4px 14px;
            border-radius: 50px;
            font-size: 11px;
            font-weight: 500;
        ...
        .table-membership .status-badge.active {
            background: #e8f5e9;
            color: #2e7d32;
        }

        .table-membership .status-badge.inactive {
            background: #fce4ec;
            color: #c62828;
        }

        .table-membership .status-badge .dot {
            width: 6px;
            height: 6px;
            border-radius: 50%;
        ...
        .table-membership .status-badge.active .dot {
            background: #4caf50;
        }

        .table-membership .status-badge.inactive .dot {
            background: #ef5350;
        }
    ...
    <div class="admin-main-content">
        <div class="list-card">
            ...
            <div class="card-header">
                <div>
                    <h4><i class="fas fa-id-card"></i> Membership List</h4>
                    <small style="opacity:0.8;">Manage all membership plans</small>
                </div>
            ...
            <div class="card-body">
                ...
                <div class="search-filter-section">
                    <div class="search-box">
                        ...
                        <input type="text" id="searchInput" placeholder="Search by plan name..." onkeyup="filterTable()">
                    </div>
                ...
                <div class="table-responsive">
                    <table class="table-membership" id="membershipTable">
                        <thead>
                            <tr>
                        ...
                        <tbody id="tableBody">
                            @forelse($memberships as $index => $membership)
                                <tr>
                                    <td class="text-center sno">{{ $memberships->firstItem() + $index }}</td>
                                    <td>
                                        @if ($membership->image)
                                            <img src="{{ asset('storage/' . $membership->image) }}" class="plan-image"
                                                alt="{{ $membership->plan_name }}">
                                        @else
                                            <img src="{{ asset('images/no-image.png') }}" class="plan-image" alt="No Image">
                                        @endif
                                    </td>
                                    <td><span class="plan-name">{{ $membership->plan_name }}</span></td>
                                    <td><span class="duration-badge"><i class="fas fa-clock"></i>
                                            {{ $membership->duration }} {{ $membership->duration_type }}</span></td>
                                    <td><span class="price-amount">₹ {{ number_format($membership->price, 2) }}</span></td>
                                    <td>
                                        @if ($membership->discount > 0)
                                            @if ($membership->discount_type == 'Percentage')
                                                <span class="discount-tag percentage">{{ $membership->discount }}%</span>
                                            @else
                                                <span class="discount-tag flat">₹
                                                    {{ number_format($membership->discount, 2) }}</span>
                                            @endif
                                        @else
                                            <span class="discount-tag none">No Discount</span>
                                        @endif
                                    </td>
                                    <td><span class="final-price">₹ {{ number_format($membership->final_price, 2) }}</span>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="action-btns">
                                            ...
                                            <button type="button" class="btn-action delete"
                                                onclick="openDeleteModal({{ $membership->id }})" title="Delete">
                                ...
                                <tr>
                                    <td colspan="9">
                                        <div class="empty-state">
                                            ...
                                            <p>Click "Add Membership" to create a new plan.</p>
                                            <a href="{{ route('admin.membership.create') }}" class="btn btn-primary"
                ...
                <div class="pagination-wrapper">
                    <div class="pagination-info">
                        Showing <strong>{{ $memberships->firstItem() ?? 0 }}</strong> to
                        <strong>{{ $memberships->lastItem() ?? 0 }}</strong> of
                        <strong>{{ $memberships->total() ?? 0 }}</strong> entries
                    </div>
    ...
    <script>
        ...
        document.getElementById('confirmDeleteBtn').addEventListener('click', function() {
            if (deleteId) {
                ...
                form.action = '/admin/memberships/' + deleteId;
                form.submit();
            }
        });
        ...
        function filterTable() {
            ...
            if (allRows.length > 0 && allRows.length === visibleRows.length) {
                ...
                td.innerHTML =
                    '<i class="fas fa-search" style="font-size:24px; display:block; margin-bottom:8px; color:#dee2e6;"></i> No memberships found matching your filters.';
                tr.appendChild(td);
