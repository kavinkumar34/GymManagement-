...
<style>
    ...
    .table-topcategories thead {
        background: var(--light-gray);
    }

    .table-topcategories thead th {
        padding: 12px 14px;
        font-weight: 600;
        font-size: 11px;
    ...
    .table-topcategories thead th.text-center {
        text-align: center;
    }

    .table-topcategories tbody td {
        padding: 10px 14px;
        vertical-align: middle;
        border-bottom: 1px solid var(--border-color);
    }

    .table-topcategories tbody tr:hover {
        background: #f8f9fa;
    }

    .table-topcategories tbody tr:last-child td {
        border-bottom: none;
    }

    .table-topcategories .sno {
        font-weight: 600;
        color: var(--gray);
        font-size: 12px;
        text-align: center;
    ...
    .table-topcategories .category-name {
        font-weight: 600;
        color: var(--dark);
    }

    .table-topcategories .gst-rate {
        font-weight: 600;
        color: #1565c0;
    }

    .table-topcategories .status-badge {
        padding: 4px 14px;
        border-radius: 50px;
        font-size: 11px;
        font-weight: 500;
    ...
    .table-topcategories .status-badge.active {
        background: #e8f5e9;
        color: #2e7d32;
    }

    .table-topcategories .status-badge.inactive {
        background: #fce4ec;
        color: #c62828;
    }

    .table-topcategories .status-badge .dot {
        width: 6px;
        height: 6px;
        border-radius: 50%;
    ...
    .table-topcategories .status-badge.active .dot {
        background: #4caf50;
    }

    .table-topcategories .status-badge.inactive .dot {
        background: #ef5350;
    }

    .table-topcategories .created-date {
        font-size: 12px;
        color: var(--gray);
        white-space: nowrap;
    }
...
<div class="admin-main-content">
    <div class="list-card">
        <div class="card-header">
            <div>
                <h4><i class="fas fa-layer-group"></i> Top Categories</h4>
                <small style="opacity:0.8;">Manage all top categories</small>
            </div>
            <div style="display:flex; gap:8px; flex-wrap:wrap; align-items:center;">
                <a href="{{ route('admin.topcategories.create') }}" class="btn btn-primary" style="background:#4a9eff; color:#fff; border:none; padding:7px 16px; border-radius:10px; text-decoration:none; display:inline-flex; align-items:center; gap:6px; font-weight:500; font-size:12px; transition:all 0.3s;">
                    <i class="fas fa-plus"></i> Add Category
                </a>
                <span style="background:rgba(74,158,255,0.2); color:#8ab4f8; padding:3px 12px; border-radius:50px; font-size:11px;">
                    <i class="fas fa-tags"></i> Total: {{ $topCategories->total() }}
                </span>
            </div>
        </div>
        ...
        <div class="card-body">
            ...
            <div class="table-responsive">
                <table class="table-topcategories" id="topCategoriesTable">
                    <thead>
                        <tr>
                    ...
                    <tbody id="tableBody">
                        ...
                        <tr>
                            <td class="text-center sno">{{ $topCategories->firstItem() + $index }}</td>
                            <td><span class="category-name">{{ $category->name }}</span></td>
                            <td class="text-center"><span class="gst-rate">{{ $category->gst_rate }}%</span></td>
                            ...
                            <td class="text-center"><span class="created-date">{{ $category->created_at->format('d M Y') }}</span></td>
                            <td>
                                <div class="action-btns">
                                    <a href="{{ route('admin.topcategories.edit', $category->id) }}" class="btn-action edit" title="Edit">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <button type="button" class="btn-action delete" onclick="openDeleteModal({{ $category->id }})" title="Delete">
                        ...
                        <tr>
                            <td colspan="7">
                                <div class="empty-state">
                                    ...
                                    <h5>No Top Categories Found</h5>
                                    <p><a href="{{ route('admin.topcategories.create') }}">Create your first category!</a></p>
                                </div>
                            </td>
                        </tr>
                        @endforelse
            ...
            <div class="pagination-wrapper">
                <div class="pagination-info">
                    Showing <strong>{{ $topCategories->firstItem() ?? 0 }}</strong> to <strong>{{ $topCategories->lastItem() ?? 0 }}</strong> of <strong>{{ $topCategories->total() ?? 0 }}</strong> entries
                </div>
...
document.getElementById('confirmDeleteBtn').addEventListener('click', function() {
    if (deleteId) {
        ...
        form.action = '/admin/topcategories/' + deleteId;
        form.submit();
    }
});
...
function filterTable() {
    ...
    if (allRows.length > 0 && allRows.length === visibleRows.length) {
        ...
        td.innerHTML = '<i class="fas fa-search" style="font-size:24px; display:block; margin-bottom:8px; color:#dee2e6;"></i> No categories found matching your filters.';
        tr.appendChild(td);
