...
<style>
    ...
    .table-categories thead th.text-center {
        text-align: center;
    }
    ...
    .table-categories .cat-placeholder {
        width: 40px;
        height: 40px;
        border-radius: var(--radius);
        background: var(--light-gray);
    ...
    .table-categories .cat-name {
        font-weight: 600;
        color: var(--dark);
    }

    .table-categories .top-cat-badge {
        padding: 3px 12px;
        border-radius: 50px;
        font-size: 11px;
        font-weight: 500;
    ...
    .table-categories .top-cat-badge .gst-tag {
    ...
    .table-categories .status-badge.active {
        background: #e8f5e9;
        color: #2e7d32;
    }

    .table-categories .status-badge.inactive {
        background: #fce4ec;
        color: #c62828;
    }

    .table-categories .status-badge .dot {
        width: 6px;
        height: 6px;
        border-radius: 50%;
    ...
    .table-categories .status-badge.active .dot {
        background: #4caf50;
    }

    .table-categories .status-badge.inactive .dot {
        background: #ef5350;
    }
    ...
    <div class="list-card">
        <div class="card-header">
            <div>
                ...
                <small style="opacity:0.8;">Manage all product categories</small>
            </div>
            <div style="display:flex; gap:8px; flex-wrap:wrap; align-items:center;">
                <a href="{{ route('admin.categories.create') }}" class="btn btn-primary" style="background:#4a9eff; color:#fff; border:none; padding:7px 16px; border-radius:10px; text-decoration:none; display:inline-flex; align-items:center; gap:6px; font-weight:500; font-size:12px; transition:all 0.3s;">
                    <i class="fas fa-plus"></i> Add Category
                </a>
                <span style="background:rgba(74,158,255,0.2); color:#8ab4f8; padding:3px 12px; border-radius:50px; font-size:11px;">
                    <i class="fas fa-tags"></i> Total: {{ $categories->total() }}
                </span>
            </div>
        </div>
        ...
        <div class="card-body">
            ...
            <div class="table-responsive">
                <table class="table-categories" id="categoriesTable">
                    <thead>
                        <tr>
                    ...
                    <tbody id="tableBody">
                        ...
                        <tr>
                            <td class="text-center sno">{{ $categories->firstItem() + $index }}</td>
                            <td>
                                @if($cat->image)
                                    <img src="{{ asset('storage/'.$cat->image) }}" class="cat-image" alt="{{ $cat->name }}">
                                @else
                            ...
                            <td>
                                <span class="top-cat-badge">
                                    <i class="fas fa-layer-group"></i> {{ $cat->topCategory->name ?? 'N/A' }}
                                    @if($cat->topCategory)
                                        <span class="gst-tag">GST: {{ $cat->topCategory->gst_rate }}%</span>
                                    @endif
                                </span>
                            </td>
                            <td>
                                <span style="background:var(--light-gray); padding:4px 10px; border-radius:6px; font-size:12px;">
                                    <i class="{{ $cat->icon ?? 'fas fa-tag' }}"></i> {{ $cat->icon ?? 'fas fa-tag' }}
                                </span>
                            </td>
                            <td class="text-center">
                                <span class="status-badge {{ $cat->is_active ? 'active' : 'inactive' }}">
                                    <span class="dot"></span> {{ $cat->is_active ? 'Active' : 'Inactive' }}
                                </span>
                            </td>
                            <td>
                                <div class="action-btns">
                                    <a href="{{ route('admin.categories.edit', $cat->id) }}" class="btn-action edit" title="Edit">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <button type="button" class="btn-action delete" onclick="openDeleteModal({{ $cat->id }})" title="Delete">
                        ...
                        <tr>
                            <td colspan="7">
                                <div class="empty-state">
                                    ...
                                    <p><a href="{{ route('admin.categories.create') }}">Create your first category!</a></p>
                                </div>
                            </td>
                        </tr>
                        @endforelse
            ...
            <div class="pagination-wrapper">
                <div class="pagination-info">
                    Showing <strong>{{ $categories->firstItem() ?? 0 }}</strong> to <strong>{{ $categories->lastItem() ?? 0 }}</strong> of <strong>{{ $categories->total() ?? 0 }}</strong> entries
                </div>
